...
function main()
{
    ...
    Render.Rect(x - 1, y - 1, 202, 61 + 15 * (names.length - 1.3), [0, 0, 0, 255]);

    Render.FilledRect(x, y, 200, 60 + 15 * (names.length - 1.3), [20, 20, 20, 180]);

    Render.Rect(x + 5, y + 5, 190, 50 + 15 * (names.length - 1.3), [2, 2, 2, 100]);
    Render.FilledRect(x + 5, y + 5, 190, 50 + 15 * (names.length - 1.3), [0, 0, 0, 250]);
    Render.FilledRect(x + 9, y + 25, 181, 3, [255, 255, 255, 255]);
    Render.String(x + 100, y + 10, 1, "spectators (" + names.length + ")", [200, 200, 200, 255], 3);

    Render.Rect(x , y , 200, 59 + 15 * (names.length - 1.3), [200, 200, 200, 100]);
    Render.Rect(x + 4, y + 4, 192, 52 + 15 * (names.length - 1.3), [200, 200, 200, 100]);

    // For each player who's spectating us, draw their names
    for (i = 0; i < names.length; i++)
    {
        Render.String(x + 100, y + 35 + 15 * i, 1, names[i], [200, 200, 200, 255], 3);
    }
